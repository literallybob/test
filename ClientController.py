...
class ClientController:
    ...
    def Start(self, timeout : int = 40):
        ...
        if isSystemLinux():
            ...
            self.Process = subprocess.Popen(["xvfb-run", "-a", "wine", self.ExecutablePath, "-t", "None", "-j", self.JoinScriptUrl, "-a", "http://www.subter.org/Login/Negotiate.ashx"], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
        else:
            self.Process = subprocess.Popen([self.ExecutablePath, "-t", "None", "-j", self.JoinScriptUrl, "-a", "http://www.subter.org/Login/Negotiate.ashx"], stdout = subprocess.PIPE, stderr = subprocess.PIPE, startupinfo = startupInfo)
