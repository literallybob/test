...
def RefillAvailableJobs():
    ...
    while len(AvailableJobs2015) < 1:
        AvailableJobs2015.append(RccController(config.RCCService2015Path, GetNextAvailablePort(config.RCCStartingComPort, config.RCCEndingComPort), KillRCCWhenFinished=False, RCCVersion="2015", useVerbose=True))
...
def GetNextAvailableRCCInstance( version : str = "2016", startKillerWatcherThread : bool = True ) -> RccController:
    ...
    GetNextRCCInstanceMutex.acquire(timeout=20)
    if version == "2015":
        if len(AvailableJobs2015) == 0:
            GetNextRCCInstanceMutex.release()
            threading.Thread(target=RefillAvailableJobs).start()
            return RccController(config.RCCService2015Path,GetNextAvailablePort(config.RCCStartingComPort, config.RCCEndingComPort), KillRCCWhenFinished=startKillerWatcherThread, RCCVersion="2015", useVerbose=True)
        AvailableInstance : RccController = AvailableJobs2015.pop(0)
        GetNextRCCInstanceMutex.release()
    elif version == "2016":
        ...
        try:
                ...
                ExecuteJSON = {
                    ...
                    "Settings": {
                        ...
                        "Arguments": [
                            ...
                            ThumbnailRequestInfo['image_x'],
                            ThumbnailRequestInfo['image_y'],
                            "http://www.subter.org/asset/?id=1785197",
            ...
            if RCCRenders >= 5 or InstanceController.PingRCC() is False:
                try:
                    ...
                    pass
                InstanceController = GetNextAvailableRCCInstance( version = "2021", startKillerWatcherThread = False)
                RCCRenders = 0
...
@app.route("/AssetValidation2015", methods=["POST"])
def AssetValidation2015():
    """
        Expected JSON Data:
        {
            "assetid": 1,
    ...
    try:
        ...
        logging.error(f"Error in AssetValidation2015: {e}")
        return "Error", 500
...
def Game():
    ...
    #  "creatorType": 1,
    #  "useNewLoadFile": true
    #  "loadfile_location": "https://www.subter.org/game/gameserver2016.lua"
    # }
    try:
        GameOpenData = request.json
...
@app.route("/Game2015", methods=["POST"])
def Game2015():
    global RunningJobs
    global NextAvilablePort
    # Expected json data:
    ...
    try:
        ...
        InstanceController : RccController = GetNextAvailableRCCInstance( version = "2015" )
        OpenJobResponse : requests.Response = InstanceController.SendOpenJobRequest(
            JobId=ServerJobId,
            Expiration=60*60*24, # 24 hours
            Cores=2,
        ...
        if RCC_UDP_Proxy is not None:
            InstanceController.BindUDPProxy(RCC_UDP_Proxy)
        logging.info(f"2015 Game server opened, jobid: {ServerJobId}, Port Forwarded: {ServerPort}, Actual Port: {ServerPort + config.PortOffset}")
        return jsonify({
            "jobid": ServerJobId,
            "port": ServerPort
        })
    except Exception as e:
        logging.error(f"Error in Game2015: {e}")
        return "Error", 500
...
def CollectDeadProcess():
    try:
        ...
        def EnumWindowsCallback( hwnd, lParam ):pass
