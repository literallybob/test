...
class RccController():
    ...
    def __init__(self, RCCExecutablePath, RCCPort = 64989, KillRCCWhenFinished : bool = True, RCCVersion : str = "2016", useVerbose : bool = True, PlaceIdStartupBypassOverwrite : int = 0 ):
        ...
        if RCCVersion not in ["2015", "2016", "2018", "2020", "2021"]:
    ...
    def PingRCCUntilTimeout(self, interval : int = 0.05 , timeout : int = 25) -> bool:
        if self.RCCProcess is None:
            return False, "RCCProcess doesn't exist!"
        if self.PollRCC() is False:
            return False, "RCC Process died!"
        t = time.time()
    ...
    def StartRCC(self, RCCPort = 64989, stdout = None, stderr = None, useVerbose : bool = False, PlaceIdStartupBypassOverwrite : int = 0):
        ...
        if isSystemLinux():
            ...
            StartingArguments = ["xvfb-run", "-a", "wine", self.RCCExecutablePath, f"{str(RCCPort)}", f"-PlaceId:{str(PlaceIdStartupBypassOverwrite)}", "-Console", "-verbose" ]
            if useVerbose:
        ...
        SuccessPoll, PollErrorMsg = self.PingRCCUntilTimeout()
        if SuccessPoll is False:
            ...
            raise Exception(f"Failed to start RCCService | {PollErrorMsg}")
